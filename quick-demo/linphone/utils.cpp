...
std::shared_ptr<linphone::Address> Utils::getMatchingLocalAddress(std::shared_ptr<linphone::Address> p_localAddress){
  ...
  // Get default proxy
  addresses.push_back(LinphoneCoreManager::getInstance()->getCore()->createPrimaryContactParsed());
  auto proxyList = LinphoneCoreManager::getInstance()->getCore()->getProxyConfigList();
