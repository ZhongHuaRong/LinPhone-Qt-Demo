...
/** @endcond */
    ...
    decaf_eddsa_448_keypair_t keypair_;
...
public:
    ...
    inline explicit PrivateKeyBase(Rng &r) DECAF_NOEXCEPT {
        FixedArrayBuffer<DECAF_EDDSA_448_PRIVATE_BYTES> priv(r);
        decaf_ed448_derive_keypair(keypair_, priv.data());
    }
    ...
    /** Create at random */
    inline ~PrivateKeyBase() { decaf_ed448_keypair_destroy(keypair_); }
    
    /** Assignment from string */
    inline PrivateKeyBase &operator=(const FixedBlock<SER_BYTES> &b) DECAF_NOEXCEPT {
        decaf_ed448_derive_keypair(keypair_, b.data());
        return *this;
    }
    ...
    inline void serialize_into(unsigned char *x) const DECAF_NOEXCEPT {
        decaf_ed448_keypair_extract_private_key(x, keypair_);
    }
    ...
    inline SecureBuffer convert_to_x() const {
        ...
        FixedArrayBuffer<DECAF_EDDSA_448_PRIVATE_BYTES> priv;
        serialize_into(priv.data());
        decaf_ed448_convert_private_key_to_x448(out.data(), priv.data());
        return out;
    }
...
public:
    ...
    inline PublicKey &operator=(const PrivateKey &p) DECAF_NOEXCEPT {
        decaf_ed448_keypair_extract_public_key(pub_.data(), p.keypair_);
        return *this;
    }
